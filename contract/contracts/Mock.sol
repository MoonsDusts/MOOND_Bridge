...
contract Mock {
    ...
    string public constant name = "MoonsDust";
    string public constant symbol = "MOOND";
    uint256 public constant decimals = 18;
    uint256 private _totalSupply;
    uint256 public tokensForSale = 1650000 * (10 ** uint256(decimals));
    uint256 public tokensSold = 1 * (10 ** uint256(decimals));
    uint256 public constant tokensPerWei = 4224;
    uint256 public constant Percent = 1000000000;
    ...
    constructor() {
        saleStartTime = block.timestamp + 15 days;
        _totalSupply = _startSupply;
        fundsWallet = 0x367db1215Bf0eFfdBc9a10d9Fd4dD9dD896f670e;
        _balances[msg.sender] = _startSupply;
        _balances[address(1)] = 0;
        emit Transfer(address(1), msg.sender, _startSupply);
    }

    receive() external payable {
        require(tokensSold < tokensForSale, "All tokens are sold");
        require(msg.value > 0, "Value must be > 0");
        require(block.timestamp < saleStartTime, "Sale Ended");
        ...
        require(
            totalTokens <= (tokensForSale).sub(tokensSold),
            "All tokens are sold"
        );
